...
public class TestsParallelCompressor {
    [Test]
    ...
    private (TimeSpan, TimeSpan) RandomLongTest() {
        ...
        try {
            start = DateTime.Now;
            BZip2ParallelCompressor compressor = new BZip2ParallelCompressor(input, output);
            compressor.StartCompression(12);
            end = DateTime.Now;
    ...
    private (TimeSpan, TimeSpan) RandomLongTestWithRepeatedValues() {
        int len = 9000000;
